...
def test_deep_b3(test_df):
    ...
    new_test = pd.concat([test1, test2])

    imgListTest = ImageList.from_df(new_test, path=path, cols='PicturePath')
    tabListTest = TabularList.from_df(new_test, cat_names=[], cont_names=cont_names, procs=procs, path=path)
    textListTest = TextList.from_df(new_test, cols='smi', path=path, vocab=vocab)
    ...
    preds, y = learnTest.get_preds(ds_type=DatasetType.Valid)
    pred = preds.numpy()[:,0]
    y_true = test_df.label.to_list()

    fpr, tpr, thresholds = metrics.roc_curve(y_true, pred, pos_label=0)
    cutoff = find_best_cutoff(fpr, tpr, thresholds)
    logging.info('best cut off is {0}'.format(cutoff))
    y_pred = list(map(lambda x: 0 if x > cutoff else 1, pred))
